...
// OtisAIRole.
//=============================================================================
class XOtisAIRole extends DOTZAIRole
	placeable;
    //config(user);
...
// wander
var protected int numLooks;

// seek
var protected Vector SeekLocation;
var protected Actor  LastSeekInstigator;
var protected float LastSeekRefreshTime;

// obstacle smashing
var protected Smashable CurrentObstacle;
var protected AttackPoint CurrentAttackPoint;
var protected int NumClearAttempts;
var protected float ClearStartTime;

// eating
var protected Actor EatTarget;
const MAX_EAT_DIST = 100;
var protected int FailedEatAttempts;
var protected float DistToEatTarget;
var int EatingTime;
const MAX_EAT_TIME = 10;

// enemy mgmt
var protected float LastHeardEnemyTime;
const REACQUIRE_DELAY       = 0.5;
const STUPID_DELAY       = 0.5;


// charging
var protected int NumChargingZombies; // using default value as a class var.
var protected const int MaxChargingZombies;
var protected const float MaxChargeDist;
var protected const float MinChargeDist;
var protected float MinTimeBetweenCharges;
var protected float MaxTimeBetweenCharges;
var protected float LastChargeTime;
var protected float ChargeStartTime;
var protected bool bIsCharging;
var protected float ChargeDuration;
var protected bool bIgnoreBumps;
var() bool bAllowedToCharge;

// other
var protected bool bActivated;
...
var() OtisAIController zombieBot;
var() protected float OtisWeaponRange;
...
function OnTakingDamage(Pawn Other, float Damage)
{
    ...
    if (ZombieAIController(Other.Controller)!=None && Other.isA('ZombiePawnBase') ){bot.enemy = Other;}
}
...
function OnThreatSpotted( Pawn threat )
{
    ...
    //if ( abs(threat.location.z - bot.pawn.location.z) > 512 ) {return;}

    if ( threat != bot.Enemy ){bot.AcquireEnemy(threat, true);}
...
function OnWitnessedKill( Controller killer, Pawn victim )
{
    ...
    if (ZombieAIController(Other.Controller)!=None && Other.isA('ZombiePawnBase') ){bot.enemy = Other;}
 */
    if ( bot.enemy == None && ( victim.IsHumanControlled() || victim.IsA('Otis') ) )
	{
		if ( killer.isA('ZombieAIController') )
		{
			//if ( TrySeekTarget(killer) ) bot.Perform_MoveToLocation( killer.Pawn.location, 128 );
			bot.enemy = killer.Pawn;
		}
    }
}
...
function bool TrySeekTarget( Actor a )
{
    ...
    if ( LastSeekInstigator == none     // if we don't have a seek target...
            || (LastSeekInstigator == a // or refreshing current target...
                    && Level.TimeSeconds - lastSeekRefreshTime > 3)
...
function bool ShouldCharge()
{
    ...
    if ( bot.Pawn.isA( 'ScuttlingZombie' ) ) {
        if ( (Level.TimeSeconds - ChargeStartTime) < MinTimeBetweenCharges || VSize(bot.Enemy.location - bot.pawn.Location) > MaxChargeDist * 1.5 )
		{
            return false;
        }
        else return true;
...
BEGIN:
	...
	{
     //               // Log( self @ "wandering"  )    ;
        if ( Level.NetMode == NM_Standalone && Level.GetLocalPlayerController().Pawn.Health > 0 )
		{
			//Level.GetLocalPlayerController().Pawn
			//seekLocation = Level.GetLocalPlayerController().Pawn.Location;
			if ( bot.actorReachable(Level.GetLocalPlayerController().Pawn) )
            bot.Perform_MoveToLocation( Level.GetLocalPlayerController().Pawn.location, 128 );
        }

        Sleep( 0.1 );
        // look around randomly...
        numLooks = RandRange( 3, 5 );
...
//DOTZMeleeWeapon
    ...
    function OnBump( Actor other )
	{
        local Pawn enemy;
        local float dist;
        dist = VSize(other.location - bot.pawn.location);

        if ( bIgnoreBumps ) return;
        enemy = Pawn( other );
        if ( enemy != none && enemy.isA('ZombiePawnBase') )
        {
			if ( bot.Pawn.Weapon.isA('DOTZMeleeWeapon') )
			{
				bIgnoreBumps = true;
...
BEGIN:
    while ( true ) 
	{
        ...
        if ( shouldCharge() ) 
		{
            ...
            bIgnoreBumps = false;
            bot.Perform_MoveToLocation( bot.pawn.location + ( (bot.enemy.location - bot.pawn.location)*0.9 ), OtisWeaponRange );
            WaitForNotification();
            EndCharge();
        }
        // normal attack
        ...
        {
            //InAttackRange(actor enemy)
            //if ( VSize(bot.enemy.location - bot.pawn.location) < MELEE_RANGE )
            //OtisWeaponRange = bot.Pawn.Weapon.EffectiveRange;
            OtisWeaponRange = 3000;
            if ( VSize(bot.enemy.location - bot.pawn.location) < OtisWeaponRange )
            {
                bIgnoreBumps = true;
            ...
            else
			{
                ...
                bot.Perform_MoveToLocation( bot.pawn.location + ( (bot.enemy.location - bot.pawn.location)*0.8 ), VSize(bot.enemy.location - bot.pawn.location) );
        ...
        */
        //if you just killed enemy, Wander!
        if ( bot.Enemy.Health <= 0 ){GotoState( 'Wander' );}
...
function DontBeStupid()
{
    ...
    player = bot.Enemy;
    if ( player == none ){GotoState( 'Wander' );}

    if (player.IsA('ZombiePawnBase') )
    {
       if ( !bot.actorReachable(bot.Enemy) ){GotoState( 'Wander' );}
       ...
       if ( abs(player.location.z - bot.pawn.location.z) < 512 ) {
           ...
           if ( player == bot.Enemy && GetStateName() != 'Attacking' &&  bot.actorReachable(bot.Enemy))
		   {
               GotoState( 'Attacking' );
           }
           // if the player isn't the enemy, but you can see and reach him, then
           // start attacking!
           if ( player != bot.Enemy && bot.CanSee(player) &&  bot.actorReachable(bot.Enemy))
           {
       ...
       SetMultiTimer( DONT_BE_STUPID_TIMER, STUPID_DELAY, false );
   }
}
...
function StartCharge() {
    ...
    XOtisAIController(zombieBot).bForceRun    = true;
    bIsCharging            = true;
...
function EndCharge() {
    ...
    default.NumChargingZombies = default.NumChargingZombies - 1;
    bIsCharging         = false;
    XOtisAIController(zombieBot).bForceRun = false;
}
...
function bool InAttackRange(actor enemy)
{
   ...
   OtisWeaponRange = 3000;
   if(VSize(bot.pawn.location - enemy.location) < OtisWeaponRange){return true;}
   else{return false;}
}
...
function PreBeginPlay()
{
   ...
   //OtisWeaponRange = bot.Pawn.Weapon.EffectiveRange;
	//OtisWeaponRange = bot.Pawn.Weapon.MaxRange;
	OtisWeaponRange = 3000;
	//AttackRange=150;
	AttackRange=3000;
	MinWanderDelay=0.100000;
	MaxWanderDelay=0.110000;
	LastSeekRefreshTime=-10.000000;
	//MaxChargingZombies=1;
	//MaxChargeDist=1750.000000;
	//MinChargeDist=100.000000;
	MinTimeBetweenCharges=0.100000;
	MaxTimeBetweenCharges=0.110000;
	bAllowedToCharge=False;
	DebugFlags=1;
